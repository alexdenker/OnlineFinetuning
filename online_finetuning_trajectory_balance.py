...
with open(os.path.join("cond_weights", "report.yaml"), "w") as file:
    yaml.dump(cfg, file)
...
y_gt = lkhd.A(x_gt)
data_consistency_gt = torch.mean((y_gt - y_noise)**2)
...
class CondSDE(torch.nn.Module):
    ...
    def forward(self, ts, x0):
        ...
        for t0, t1 in zip(ts[:-1], ts[1:]):
            ...
            kldiv_term1 = kldiv_term1 - 0.5 * f_sq * diffusion.pow(2) * dt  # only tak 20% of time steps
...
wandb_kwargs = {
        ...
        "config": cfg,
        "name": "trajectory balance (flowers)",
...
with wandb.init(**wandb_kwargs) as run:
    ...
    if cfg["optim_k"] == "SGD":
        optimizerk = torch.optim.SGD([sde_model.k], lr=cfg["lrk"])
    else:
        optimizerk = torch.optim.Adam([sde_model.k], lr=cfg["lrk"])
    ...
    ax1.imshow((x_gt[0].permute(1,2,0).cpu() + 1.)/2.)
    ax1.axis("off")

    ax2.set_title("y")
    ax2.imshow((y_noise[0].permute(1,2,0).cpu().numpy() + 1.)/2.)
    ax2.axis("off")

    ax3.set_title("Adjoint(y)")
    ax3.imshow((Aty[0].permute(1,2,0).cpu() +1.)/2.)
    ax3.axis("off")
    ...
    for step in tqdm(range(cfg["num_steps"])):
        ...
        loss_data = 0.75/noise_level**2*data_fit  # * 1/2
        loss_kl = kldiv_term1 + kldiv_term2 + kldiv_term3
        rnd = loss_data + kldiv_term1 + kldiv_term2 + kldiv_term3

        #print(rnd)
        # filter out to large loss values 
        cut_off_val = 500000
        rnd_filtered = rnd[rnd < cut_off_val]
        print(rnd)
        if len(rnd_filtered) > int(0.3*batch_size):
            mean_rnd = rnd_filtered.mean() 
            
            tb_loss = torch.mean((rnd_filtered - sde_model.k)**2)

            #print("TB loss: ", tb_loss)
            original_loss = (loss_data[rnd < cut_off_val] - kldiv_term1[rnd < cut_off_val]).mean()
            ...
            wandb.log( 
                        ...
                        "train/trajectory_balance_loss": tb_loss.item(),
                        ...
                        "train/k": sde_model.k.item(), #sde_model.k.item(),
                        "train/mean_rnd": mean_rnd.item(),
                        "step": step}
                    ) 
        else:
            print("Too many loss values were filtered out. Draw new samples")

        torch.save(sde_model.cond_model.state_dict(), "cond_weights/cond_model.pt")
        torch.save(sde_model.time_model.state_dict(), "cond_weights/time_model.pt")
        ...
        if step % cfg["log_img_freq"] == 0 and step > 0:
            ...
            for idx, ax in enumerate(axes.ravel()):
                if idx == 0:
                    to_plt = (x_gt[0].permute(1,2,0).cpu().numpy() + 1.)/2.
                    ax.imshow(np.clip(to_plt, 0, 1))
                    ax.set_title("GT")
                elif idx == 1:
                    to_plt = (y_noise[0].permute(1,2,0).cpu().numpy() + 1.)/2.
                    ax.imshow(np.clip(to_plt, 0, 1))
                    ax.set_title("y (4x ds)")
                else:
                    to_plt = (xt[-1][idx-1].permute(1,2,0).detach().cpu().numpy() + 1.)/2.
                    ax.imshow(np.clip(to_plt, 0, 1))
            ...
            psnrs = [] 
            for i in range(xt[-1].shape[0]):
                psnr = peak_signal_noise_ratio(x_target_val.detach().cpu().numpy()[i], xt[-1].detach().cpu().numpy()[i], data_range=2.)
                psnrs.append(psnr)
            #print("mean PSNR (val): ", np.mean(psnrs))


            mean_sample = xt[-1].cpu().mean(dim=0)
            mean_sample = torch.clamp(mean_sample, -1, 1)

            y_sim = lkhd.A(xt[-1])

            data_consistency = torch.mean((y_sim - y_noise)**2, dim=(1,2,3))
            ...
            ax1.hist(diff_to_mean.ravel().numpy(), bins="auto", alpha=0.75)
            ax1.set_title("| x - x_mean|")
            ax1.vlines(diff_to_gt.numpy(), 0, 10, label="| x_mean - x_gt|", colors='r')
            ax1.legend()

            ax2.hist(data_consistency.cpu().numpy().ravel(), bins="auto", alpha=0.75)
            ax2.set_title("|| A(x) - y_noise||")
            ax2.vlines(data_consistency_gt.cpu().numpy(), 0, 10, label="|| A(x_gt) - y_noise||", colors='r')
            ax2.legend()
