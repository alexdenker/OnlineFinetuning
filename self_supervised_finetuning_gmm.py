...
pos_in = torch.from_numpy(pos.T).to(target.device)
...
class CondSDE(torch.nn.Module):
    ...
    def predict_noise(self, xt, t):
        ...
        with torch.no_grad():
            ...
            x0hat = (xt + marginal_prob_std_expanded**2*s_pretrained)/marginal_prob_mean_expanded
            
        h_trans = self.cond_model(torch.cat([xt, x0hat], dim=1), t) 
    ...
    def forward(self, ts, xT):
        ...
        for t0, t1 in zip(ts[:-1], ts[1:]):
            ...
            kldiv_term1 = kldiv_term1 + 0.5 * f_sq.detach() * diffusion_expanded.pow(2) * dt 
            kldiv_term2 = kldiv_term2 + diffusion_expanded * f_w
...
for i in tqdm(range(cfg["num_steps"])):
    ...
    with torch.no_grad():
        xt, kldiv1, kldiv2 = sde_model.forward(ts, xT)

        loss_data = r(xt[-1])

        # compute importance weights
        log_iw = -loss_data  + kldiv1 + kldiv2
    ...
    print(xt[-1].shape)
    x0_after_resampling = xt[-1][resampling]
    

    fig, (ax1, ax2) = plt.subplots(1,2)
    ax1.scatter(samples_target.cpu().numpy()[:,0], samples_target.cpu().numpy()[:,1], c='r', label="prior samples", alpha=0.1)
    im = ax1.scatter(xt[-1].detach().cpu().numpy()[:,0], xt[-1].detach().cpu().numpy()[:,1], c=resampling_ratio.cpu().numpy(), label="fine-tuned samples")
    fig.colorbar(im, ax=ax1)
    ax1.legend()
    
    ax2.scatter(samples_target.cpu().numpy()[:,0], samples_target.cpu().numpy()[:,1], c='r', label="prior samples", alpha=0.1)
    ax2.scatter(x0_after_resampling.detach().cpu().numpy()[:,0], x0_after_resampling.detach().cpu().numpy()[:,1], label="samples after resampling")
    plt.show()
    ...
    for train_iter in range(40):
        ...
        loss = finetuning_score_based_loss_fn(x0_after_resampling, sde_model, sde)
        loss.backward()
    ...
    with torch.no_grad():
        ...
        ts = np.linspace(np.sqrt(1e-3), 1., 500)[::-1].copy()
        ts = torch.from_numpy(ts).to(device)**2

        with torch.no_grad():
            xt, _, _ = sde_model.forward(ts, xT)

        fig, ax = plt.subplots(1,1)
        ax.scatter(samples_target.cpu().numpy()[:,0], samples_target.cpu().numpy()[:,1], c='r', label="prior samples", alpha=0.1)
        im = ax.scatter(xt[-1].detach().cpu().numpy()[:,0], xt[-1].detach().cpu().numpy()[:,1], label="fine-tuned samples")
        ax.legend()
        plt.show()
